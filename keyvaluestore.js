		...
		if (self.cache.get(search))
			...
			db.query(params, function(err, data) {
				if (err || data.Items.length == 0)
					callback(err, null, search);
				else {
					self.cache.set(search, data.Items[0].value.S);
					callback(err, data.Items[0].value.S, search);
				}
			});
		}
	};
...
//			console.log(data);
	 ...
	 * @param value
	 */
	keyvaluestore.prototype.put = function(keyword, value, index, callback) {
		var self = this;
	...
	keyvaluestore.prototype.getIndexValues = function(search, callback) {
		var self = this;
		console.log("IN KVS in getIndexValues, before items");
		
		
			console.log("IN KVS in ELSE");
			var params = {
					KeyConditions: {
						keyword: {
							ComparisonOperator: 'EQ',
			...
			db.query(params, function(err, data) {
				if (err || data.Items.length == 0)
					...
					//console.log("in getIndexValues, before items");
					var items = [];
					for (var i = 0; i < data.Items.length; i++) {
						var map = {
								'keyword': search,
						...
						items.push(JSON.stringify(map));
					}
					self.cache.set(search, items.toString());
					callback(err, items);
				}
			});
